...
def get_config():
    config = {
        ...
        "intraday_multipliers": ["5", "30", "240", "1D", "1W"],  # 是提示 UI 展示什么分钟选项（不一定强制）
        "supported_resolutions": ["5", "30", "240", "1D", "1W"]
        # ["D","2D","3D","W","3W","M","6M"]
    }
...
def search():
    ...
    sql = 'select \"Symbol\", \"Name\", \"CodeType\" as \"type\" from stock_list where 1=1'
    pattern = f'%{query}%'
    if query is not None:
        sql = sql + ' and \"Symbol\" like %(pattern)s'
    if type is not None and '' != type:
        sql = sql + ' and \"CodeType\" = \''+type+'\''
    sql = sql + ' order by \"Symbol\" limit ' + limit
...
def symbols():
    ...
    sql = f'select * from stock_list where \"Symbol\" = \'{symbol}\''
    ...
    jsonstr = {
        ...
        "name": symbol,
        "has_intraday": True,  # 是必须的才能支持分钟线
        "intraday_multipliers": ["5", "30", "240", "1D", "1W"],
        "pointvalue": 1,
        "pricescale": 100,
        "session": "0730-1830",
...
def history():
    ...
    countSql = f'SELECT count(1) FROM stock_{resolution} where \"Ticker\" = \'{symbol}\' and \"{field}\" <= TO_TIMESTAMP({toTime})::DATE and \"{field}\" >= TO_TIMESTAMP({fromTime})::DATE'
    ...
    sql = f'select "Datetime","Open","Close","High","Low","Volume" from stock_{resolution} where \"Ticker\" = \'{symbol}\' and \"{field}\" <= \'{formattedToTime}\'  and \"{field}\" >= \'{formattedFromTime}\'  order by \"{field}\" desc limit {countback}'
    df = pd.read_sql(sql, engine)
    if df.shape[0] == 0:
        return {"s": "no_data"}
...
def top_volume():
    ...
    sql = (f'select * from stock_1d sd where \"Datetime\" >= \'{datetimeGT}\' '
           f'and \"Datetime\" <= \'{datetimeET}\' order by sd."Volume" desc limit 100')
    df = pd.read_sql(sql, engine)
    json_str = df.to_json(orient='records', force_ascii=False)
    py_obj = json.loads(json_str)
    result = {
...
def third_bs_query():
    ...
    datetimeGT = request.args.get('datetimeGT', '')
    datetimeET = request.args.get('datetimeET', '')
    hot_str = request.args.get('hot', default=None)
    hot = str(hot_str).lower() in ['true', '1', 'yes', 'on']

    if resolution == "1d":
        sql = (f'select mc.*,'
               f'(select shs1.\"Volume\" from stock_hot_selected shs1 where date(mc.\"endDate\") = date(shs1.\"Datetime\" ) and shs1.\"Ticker\" = mc.\"Ticker\") as Volume,'
               f'(select shs1.\"MA10\" from stock_hot_selected shs1 where date(mc.\"endDate\") = date(shs1.\"Datetime\" ) and shs1.\"Ticker\" = mc.\"Ticker\") as MA10 '
               f' from moer_cl_{resolution} mc where mc.\"class_type\" is not null and mc.\"endDate\" >= \'{datetimeGT}\' and mc.\"endDate\"  <= \'{datetimeET}\' ')
        if hot:
            sql += f' and mc.\"Ticker\" in(select shs."Ticker" from stock_hot_selected shs where shs."Volume" >shs."MA10" and shs."Datetime" >= \'{datetimeGT}\' and shs."Datetime" <= \'{datetimeET}\') '
    ...
    df = pd.read_sql(sql, engine)
    df['BeiBi'] = df['volume']/df['ma10']
    df['startDate'] = df['startDate'].dt.tz_localize('America/Chicago').astype('int64') // 10 ** 9
    df['endDate'] = df['endDate'].dt.tz_localize('America/Chicago').astype('int64') // 10 ** 9
    json_str = df.to_json(orient='records', force_ascii=False)
    py_obj = json.loads(json_str)
    result = {
        "data": py_obj
...
def chan():
    ...
    symbol = request.args.get('symbol', '').upper()
    if resolution == "1d":
        sql = f'select * from moer_cl_{resolution} where  \"Ticker\" = \'{symbol}\' order by "startDate" asc'
    else:
        if resolution == "1w":
            resolutionT = "1wk"
        if resolution == "240":
